...
// ── Allowed CAD/document/image extensions ─────────────────────────────────────
const ALLOWED_EXT = /\.(pdf|dxf|dwg|step|stp|jpg|jpeg|png|webp|gif)$/i;
...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    if (ALLOWED_EXT.test(file.originalname)) {
      ...
      cb(new Error("Invalid file type. Allowed: .pdf .dxf .dwg .step .stp .jpg .jpeg .png .webp .gif"));
    }
  },
});
