 ...
 * Two-column layout: product image on the left, dimensions table on the right.
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col lg:flex-row gap-10 xl:gap-14 items-stretch">
        ...
        {/* Right — dimensions table only */}
        <div className="flex-1 min-w-0">
          <div className="border border-slate-200 border-t-2 border-t-[#001f4d] overflow-hidden h-full">
            <Table>
              <TableHeader>
                <TableRow className="bg-[#001f4d] hover:bg-[#001f4d] border-b-0">
                  {hasSpec && (
                    <TableHead className="h-auto px-6 py-4 font-mono font-black text-[11px] uppercase tracking-widest text-white">
                      {col0Label}
                    </TableHead>
                  )}
                  <TableHead className="h-auto px-6 py-4 font-mono font-black text-[11px] uppercase tracking-widest text-white">
              ...
              <TableBody>
                {blade.standardDimensions.map((row, i) => (
                  <TableRow
                    key={i}
                    className={i % 2 === 0 ? "bg-white hover:bg-slate-50" : "bg-slate-50/70 hover:bg-slate-100"}
                  >
                    {hasSpec && (
                      <TableCell className="px-6 py-4 font-mono text-sm text-slate-500">
                        {row.spec ?? "—"}
                      </TableCell>
                    )}
                    <TableCell className="px-6 py-4 font-mono font-black text-base text-[#001f4d]">
              ...
              <TableCaption className="text-left px-6 py-2 border-t border-slate-100 bg-slate-50/50">
